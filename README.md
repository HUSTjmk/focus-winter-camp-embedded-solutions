...
- TASK2：**基础任务**  
  - TASK2.0：**机械小狗的建模与装配**
    - TASK 2.0.0: **小狗本体与履带的建模**\
      ...
      (1)：小狗本体的建模应有预留的可以给电机、LCD屏幕以及电路板固定的孔位，不允许存在没有固定的电子元器件。整个小狗体积限制在256mm×256mm×200mm，不要超过这个限制  
...
### 软件篇
- 以下有3个TASK,其中TASK1为环境相关，TASK2为任务要求，TASK3为心得体会。其中任务前带*号的是附加任务，留给学有余力的同学完成。建议基础任务由两人分工完成，一个人调语音、手势、屏幕等传感器，另外一人调电机超声波等运动控制（当然只是建议）。大一同学至少完成基础任务，大二同学在完成基础任务的基础上再至少完成一个附加任务。
- 对于”发给我们“这类话语，统一发给jmk（仅限嵌软任务）
...
- 每段TASK最后都写明了都要录视频，如果做不到录视频那一步，请记录到最新完成的那一步并拍视频/照片，如果遇到视频过大传不上github,可以一起打包成**压缩包**发给jmk,**每个视频要有清晰的命名🙏🏼** 
...
- TASK1：**环境搭建** 
  - TASK1.0：**环境搭建** 
    - TASK1.0.0：**搭建stm32的环境**   
      千里之行，始于足下！开发一块板子你首先需要一个开发环境。对于stm32开发来说，主流有寄存器/标准库/HAL库等开发方式。我们推荐HAL库开发，当然你也可以参考江科大等优秀的历程使用其他方式开发。下面是我们推荐的参考教程：
    ...
    - TASK1.0.1：**点亮一个LED灯泡💡**  
      看来你已经配完了环境，太棒了！但是环境配好了不一定代表stm32的代码可以很顺利地编译、下载与调试呢~~实践是检验真理的唯一标准，接下来我们试试点亮板载的LED（PC13引脚），看看我们的环境是否完全搭建好了吧！**记得录一段点亮LED的视频并上传给我们哦！** 
- TASK2：**任务要求**     
  - TASK2.0：**Movement**     
    ...
    - TASK2.0.0：**看看学长为你们准备的资料** 
      ...
      网址：对应教程的pwm章节 AND [CSDN上的一篇DRV8837教程](https://blog.csdn.net/weixin_45229061/article/details/142480529)  
    - TASK2.0.1：**让电机动起来**   
  ...
  - TASK2.1：**Audio**
    ...
    - TASK2.1.0：**看看学长为你们准备的资料**  
      ...
      网址：对应教程的串口章节 AND [机芯智能B站官方教程](https://www.bilibili.com/video/BV1DG4y1u7mb/)
    - TASK2.1.1：**实现语音播放与离线语音识别**   
      你需要查阅SU-03T手册，登陆他们的官网烧录固件，实现语语音播报功能与语音识别功能~   
    - TASK2.1.2：**用离线词实现运动控制吧**  
      好了，你现在已经学会了怎么输入输出语音了，真棒👍！想必你/你得力的队员已经把运动控制搞定了，现在给桌宠说一句”前进”，让它向前冲吧！（其他也行）**这里录一段视频，展现上面的效果！**
  ...
  - TASK2.2：**Display**  
    ...
    - TASK2.2.0：**看看学长为你们准备的资料**  
      ...
      网址：[江科大OLED教程](https://www.bilibili.com/video/BV1EN41177Pc/) AND [HAL库版本](https://www.bilibili.com/video/BV1M6ioYzE58)
    - TASK2.2.1：**点亮一个屏幕**  
      让你的OLED亮起来！
  ...
  - TASK2.3：**Ultrasonic**  
    ...
    - Task2.3.0：**看看学长为你们准备的资料**  
      ...
      网址：[CSDN上的一篇教程](https://blog.csdn.net/qq_16519885/article/details/113778828)
    - TASK2.3.1：**实现超声波测距功能**  
      使用HC-SR04实现最基础的测距功能。
  ...
  - TASK2.4：**Sensor**  
    ...
    - *TASK2.4.2：**手势传感的另一种solution** 
      之前的手势传感是使用传统的物理方法，但其实我们还可以采用摄像头+视觉模型进行开发。我们回到[语音交互的另一种solution](#audio-another-solution)，我们的老朋友ESP32支持多种摄像头接口，并且支持轻量的深度学习模型ESP-DL。想要解锁该任务，请在保证之前**基础任务和其他附加任务完成**的前提下联系jmk，他会觉得你十分高能🔥
  - TASK2.5：**Combination** 
...
- TASK3：**心得体会**
  - TASK3.0：**心得体会**
    - TASK3.0.0：**花上一点时间，谈谈你对这次冬令营的心得体会。**  
      包括但不限于：个人在技术、心态上的成长，对团队协作的理解，以及本次任务的反馈意见等。传到你的git仓库里，**没搞电路的那个人必写一篇，搞了电路又搞软件的人可以选择性写一篇，一个人干完了所有的也要写一篇并收到我们的同情😇**，字数不限，格式不限
